...
type Conn struct {
	...
	sessionsByChannel   map[uint16]*session.Session
...
func New(netConn net.Conn, opts *ConnOptions) (*Conn, error) {
	c := &Conn{
...
func (c *Conn) Close() error {
	...
	err := c.Err()
	var connErr *exported.ConnectionError
	if errors.As(err, &connErr) && reflect.ValueOf(connErr).IsZero() {
...
func (c *Conn) Err() error {
	...
	return exported.NewConnectionError(c.err)
}

func (c *Conn) NewSession(opts *session.SessionOptions) (*session.Session, error) {
...
func (c *Conn) DeleteSession(s *session.Session) {
	c.sessionsByChannelMu.Lock()
	defer c.sessionsByChannelMu.Unlock()
...
func (c *Conn) mux() {
	var (
		...
		sessionsByRemoteChannel = make(map[uint16]*session.Session)
	)
