	...
	case state := <-done:
		if state, ok := state.(*encoding.StateRejected); ok {
			if s.detachOnRejectDisp() {
				return &exported.DetachError{state.Error}
			}
			return state.Error
		}
		return nil
...
func (s *Sender) send(ctx context.Context, msg *SenderMsg) (chan encoding.DeliveryState, error) {
	...
	var (
		...
		senderSettled  = sndSettleMode != nil && (*sndSettleMode == encoding.ModeSettled || (*sndSettleMode == encoding.ModeMixed && msg.SendSettled))
...
func NewSender(target string, s Session, opts *SenderOptions) (*Sender, error) {
	l := &Sender{
		l: Link{
			key:      Key{shared.RandString(40), encoding.RoleSender},
			session:  s,
...
func (s *Sender) Attach(ctx context.Context, session Session) error {
	...
	if senderSettleModeValue(s.l.senderSettleMode) != encoding.ModeSettled && receiverSettleModeValue(s.l.receiverSettleMode) == encoding.ModeSecond {
	...
	for {
		...
		case tr := <-outgoingTransfers:
			...
			for {
				...
				case <-s.l.close:
					s.l.err = exported.ErrLinkClosed
					return
...
func (s *Sender) detachOnRejectDisp() bool {
	...
	if s.detachOnDispositionError && (s.l.receiverSettleMode == nil || *s.l.receiverSettleMode == encoding.ModeFirst) {
		return true
	}
	return false
}
