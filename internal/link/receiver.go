...
func (r *Receiver) RejectMessage(ctx context.Context, msg *Message, e *encoding.Error) error {
	if !msg.shouldSendDisposition() {
		return nil
	}
...
func (r *Receiver) closeWithError(de *encoding.Error) error {
...
func (r *Receiver) sendDisposition(first uint32, last *uint32, state encoding.DeliveryState) error {
	fr := &frames.PerformDisposition{
		...
		First:   first,
		Last:    last,
		Settled: r.l.receiverSettleMode == nil || *r.l.receiverSettleMode == encoding.ModeFirst,
		State:   state,
	}
...
func (r *Receiver) messageDisposition(ctx context.Context, msg *Message, state encoding.DeliveryState) error {
	var wait chan error
	if r.l.receiverSettleMode != nil && *r.l.receiverSettleMode == encoding.ModeSecond {
...
func NewReceiver(source string, s Session, opts *ReceiverOptions) (*Receiver, error) {
	l := &Receiver{
		l: Link{
			key:      Key{shared.RandString(40), encoding.RoleReceiver},
			session:  s,
	...
	if opts.RequestedSenderSettleMode != nil {
		if rsm := *opts.RequestedSenderSettleMode; rsm > encoding.ModeMixed {
...
func (r *Receiver) mux() {
	...
	for {
		...
		case <-r.receiverReady:
			continue
		case <-r.l.close:
			r.l.err = exported.ErrLinkClosed
			return
...
func (r *Receiver) muxReceive(fr frames.PerformTransfer) error {
	if !r.more {
		...
		if fr.DeliveryID == nil {
			return r.closeWithError(&encoding.Error{
	...
	// send to receiver
	if receiverSettleModeValue(r.l.receiverSettleMode) == encoding.ModeSecond {
