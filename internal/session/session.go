...
type Conn interface {
	...
	SendFrame(fr frames.Frame) error
...
type Session struct {
	...
	linksByKey map[link.Key]*link.Link // mapping of name+role link
...
func New(c Conn, channel uint16, opts *SessionOptions) *Session {
	s := &Session{
		conn:           c,
		channel:        channel,
		...
		linksByKey:     make(map[link.Key]*link.Link),
...
func (s *Session) Close(ctx context.Context) error {
	...
	if s.err == exported.ErrSessionClosed {
		return nil
	}
	return s.err
}
...
func (s *Session) SetRemoteChannel(rc uint16) {
	s.remoteChannel = rc
}
...
func (s *Session) NextDeliveryID() uint32 {
	return atomic.AddUint32(&s.nextDeliveryID, 1)
}

func (s *Session) TX() chan<- frames.FrameBody {
	return s.tx
}

func (s *Session) TXTransfer() chan<- *frames.PerformTransfer {
	return s.txTransfer
}

func (s *Session) RX() chan<- frames.Frame {
	return s.rx
}
...
func (s *Session) NewReceiver(ctx context.Context, source string, opts *link.ReceiverOptions) (*link.Receiver, error) {
	r, err := link.NewReceiver(source, s, opts)
	if err != nil {
		return nil, err
	}
...
// opts: pass nil to accept the default values.
func (s *Session) NewSender(ctx context.Context, target string, opts *link.SenderOptions) (*link.Sender, error) {
	l, err := link.NewSender(target, s, opts)
	if err != nil {
		return nil, err
	}
...
func (s *Session) mux(remoteBegin *frames.PerformBegin) {
	defer func() {
		s.conn.DeleteSession(s)
		if s.err == nil {
			s.err = exported.ErrSessionClosed
		}
	...
	var (
		links                     = make(map[uint32]*link.Link) // mapping of remote handles to links
	...
	for {
		...
		case fr := <-s.rx:
			...
			case *frames.PerformAttach:
				...
				s.linksMu.RLock()
				link, linkOk := s.linksByKey[link.Key{Name: body.Name, Role: !body.Role}]
				s.linksMu.RUnlock()
				if !linkOk {
				...
				link.SetRemoteHandle(body.Handle)
				links[link.GetRemoteHandle()] = link
			...
			case *frames.PerformTransfer:
				...
				if !body.Settled && body.DeliveryID != nil && link.RSM() != nil && *link.RSM() == encoding.ModeSecond {
			...
			case *frames.PerformDetach:
				...
				delete(links, link.GetRemoteHandle())
				delete(deliveryIDByHandle, link.GetHandle())
...
func (s *Session) AllocateHandle(l *link.Link) error {
	s.linksMu.Lock()
	defer s.linksMu.Unlock()

	// Check if link name already exists, if so then an error should be returned
	key := l.Key()
	existing := s.linksByKey[key]
	if existing != nil {
		return fmt.Errorf("link with name '%v' already exists", key.Name)
	}
	...
	l.SetHandle(next)     // allocate handle to the link
	s.linksByKey[key] = l // add to mapping

	return nil
}

func (s *Session) DeallocateHandle(l *link.Link) {
	s.linksMu.Lock()
	defer s.linksMu.Unlock()

	delete(s.linksByKey, l.Key())
	s.handles.Remove(l.GetHandle())
	close(l.rx)
}

func (s *Session) muxFrameToLink(l *link.Link, fr frames.FrameBody) {
	select {
	case l.RX() <- fr:
